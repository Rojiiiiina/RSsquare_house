...
class Cust_Win:
    def __init__(self, root):
        ...
        self.root.geometry("1160x720+220+150")
        # variables

        self.var_ref = StringVar()
        x = random.randint(950, 8850)
        ...
        # title
        lbl_title = Label(self.root, text="ADD CUSTOMER DETAIL", font=("times new roman", 18, "bold"), bg="black",
        ...
        # mother name
        lblfname = Label(labelframeleft, font=("arial", 12, "bold"), text="Father Name:", padx=2, pady=6)
        ...
        combo_gender = ttk.Combobox(labelframeleft, textvariable=self.var_gender, font=("arial", 12, "bold"), width=27,
                                    state="readonly")
        combo_gender = ("Male", "Female", "Other")
        ...
        combo_id = ttk.Combobox(labelframeleft, textvariable=self.var_id_proof, font=("arial", 12, "bold"), width=27,
                                state="readonly")
        combo_id["value"] = ("Citizenship", "passport")
        combo_id.current(0)
        ...
        Table_frameleft = LabelFrame(self.root, bd=2, relief=RIDGE, text="View Details And Search System",
                                     font=("times new roman", 12, "bold"), padx=2, )
        ...
        combo_Search = ttk.Combobox(Table_frameleft, textvariable=self.search_var, font=("arial", 12, "bold"), width=15,
                                    state="readonly")
        ...
        btnsearch = Button(Table_frameleft, text="Search", command=self.search, font=("arial", 11, "bold"), bg="black",
                           fg="gold", width=9)
        btnsearch.grid(row=0, column=3, padx=1)

        btnShowAll = Button(Table_frameleft, text="Show All", command=self.fetch_data, font=("arial", 11, "bold"),
        ...
        self.Cust_Details_Table = ttk.Treeview(details_table, column=(
            "name", "father", "gender", "mobile", "email", "nationality", "idproof", "idnumber", "address"),
    ...
    def add_data(self):
        if self.var_mobiles.get() == "" or self.var_father.get() == "":
            ...
            try:
                ...
                my_cursor.execute("insert into customer values(%a)", (
    ...
    def update(self):
        if self.var_mobile.get() == "":
            ...
            my_cursor = conn.cursor()
            my_cursor.execute(
                "update customer set Name=%a,Father=%a,Gender%a,PostCode=%a,Mobile=%a,Email=%a,Nationality=%a,Idprrof=%a,Idnumber=%a,Address=%a where Ref=%a",
                (
    ...
    def mDelete(self):
        mDelete = messagebox.askyesno("RSsquare House", "Do you want to delete this customer",
                                      parent=self.root)
        if mDelete > 0:
            conn = mysql.connector.connect(owner="ownerhere", username="sweethome", password="p@ssword12",
                                           database="housing")
            my_cursor = conn.cursor()
            query = "delete from customer where Ref=%a"
